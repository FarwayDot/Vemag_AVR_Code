...
/*Defines ------------------------------------------------------------------------*/
/**
 * @brief LECTURA DE SENSORES
 * NA -> al micro le llega 5VDC (1) por defecto
 * NC -> al micro le llega 0VDC (0) por defecto
 */
#define NC          0
#define NA          1

#define STATE_H_OIL NA
#define STATE_L_OIL NA
#define STATE_S_PRT NA
...
//->begin	
#if (STATE_H_OIL  == NC)
#warning "H_OIL->NC Selected"
	AlarmEdge_H_OIL = PIN_H_OIL & (1 << H_OIL);
#elif (STATE_H_OIL == NA)
#warning "H_OIL->NA Selected"
	AlarmEdge_H_OIL = !(PIN_H_OIL & (1 << H_OIL));
#else
#error "Selecciona el valor correcto"
#endif

#if (STATE_L_OIL == NC)	
#warning "L_OIL->NC Selected"
	AlarmEdge_L_OIL = PIN_L_OIL & (1 << L_OIL);
#elif(STATE_L_OIL == NA)
#warning "L_OIL->NA Selected"
	AlarmEdge_L_OIL = !(PIN_L_OIL & (1 << L_OIL));
#else
#error "Selecciona el valor correcto"
#endif
...
#if (STATE_S_PRT == NC)
#warning "S_PRT->NC Selected"
	AlarmEdge_S_PRT = PIN_S_PRT & (1 << S_PRT);
#elif (STATE_S_PRT == NA)
#warning "S_PRT->NA Selected"
	AlarmEdge_S_PRT = !(PIN_S_PRT & (1 << S_PRT));
#else
#error "Selecciona el valor correcto"
#endif
...
*/
	...
	currentButton_AlarmReleasedEdge = currentButton_AlarmEdge;
...
void SensorsAlarmState_Actions(void){
	...
	LCD_Goto_XY(1, 1);
	LCD_Put_String("    !PRECAUCION!    ");

	
	if (counterError == 1) {
		LCD_Goto_XY(1, 2);
		LCD_Put_String("Revisar los sensores");
	}
	...
	if (counterError == 3 ){
		...
		if (AlarmEdge_S_PRT && (counterErrorMessage == 3)){
			LCD_Goto_XY(1, 2);
			LCD_Put_String("Sensor de Proteccion");
			counterErrorMessage = 0;
		}
	}
